...
function Offre() {
    ...
    useEffect(() => {
        const fetchOffres = async () => {
            const offresCollection = collection(db, 'appel_offre');
            const snapshot = await getDocs(offresCollection);
            const offresList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        <div dir={i18n.language === "ar" ? "rtl" : "ltr"} className="block my-2 w-full max-w-[999px] p-6 bg-white border border-gray-200 rounded-lg shadow">
            ...
            {offres.map(el => (
                <div key={el.id} className="mb-4">
                    ...
                    <p className="mb-2">{el.descriptions[i18n.language]}</p>
