...
const AdminJourInformation = () => {
    ...
    try {
      ...
      if (files.length > 0) {
        const uploadPromises = files.map(async (file) => {
          const storageRef = ref(storage, `jours_information/${file.name}`);
      ...
      if (editId) {
        const articleRef = doc(firestore, "jours_information", editId);
        ...
        const docRef = await addDoc(
          collection(firestore, "jours_information"),
          {
            titles,
            descriptions,
            images: downloadURLs,
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, "jours_information", id));
  ...
  return (
    <div className="p-4 bg-gray-100 rounded-md shadow-md">
      <h1 className="text-center mb-5">{t("Journées d'information")}</h1>
...
export default AdminJourInformation;
