...
const AppelOffreAdmin = () => {
    ...
    useEffect(() => {
        const fetchArticles = async () => {
            const articlesCollection = collection(db, 'appel_offre');
    ...
    const handleUpload = async () => {
        if (!titles.ar  || !titles.fr || !titles.en ||  (files.length === 0 && !editId)) {
        ...
        try {
            ...
            if (files.length > 0) {
                const uploadPromises = files.map(async (file) => {
                    const storageRef = ref(storage, `appel_offre/${file.name}`);
            ...
            if (editId) {
                const articleRef = doc(firestore, 'appel_offre', editId);
                ...
                const docRef = await addDoc(collection(firestore, 'appel_offre'), {
                    titles,
                    descriptions,
                    images: downloadURLs
                });
    ...
    const handleDelete = async (id) => {
        try {
            await deleteDoc(doc(db, 'appel_offre', id));
    ...
    return (
        <div className="p-4 bg-gray-100 rounded-md shadow-md">
            <h1 className='text-center mb-5'> {t("Appel d'offres")}</h1>
                        ...
                        <div className="flex flex-column justify-around border border-grey shadow p-2 items-center border-b border-gray-200 px-4">
                            ...
                            <div className='text-center w-100'>
                                {article.images && article.images.map((image, index) => (
                                    <iframe key={index} style={{ maxWidth:"100%", maxHeight: '20vh' }} src={image} alt={`Article ${index}`} />
                                ))}
                            </div>
                            <div>
                                <button onClick={() => handleEdit(article)} className="mx-2 w-100 my-2 btn btn-primary">{t('Edit')}</button>
